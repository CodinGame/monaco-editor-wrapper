    ...
    for (const selection of editor.selections) {
      for (let lineNumber = selection.start.line; lineNumber <= selection.end.line; lineNumber++) {
        ...
        if (expectedIndent == null) {
          ...
          const expectedIndentCount =
            Math.floor((nonSpacePosition - lastFixedPosition) / indentWidth) + 1
  ...
  await editor.edit((builder) => {
    for (const selection of editor.selections) {
      for (let lineNumber = selection.start.line; lineNumber <= selection.end.line; lineNumber++) {
        ...
        const prevFixedPosition =
          fixedPositions
            .slice()
            .reverse()
            .find((p) => p < nonSpacePosition) ?? 0
        ...
        if (prevFixedPosition === lastFixedPosition) {
          ...
          const expectedIndentCount = Math.floor(
            (nonSpacePosition - lastFixedPosition - 1) / fixedIndentWidth
          )
        ...
        builder.delete(new vscode.Range(lineNumber, 0, lineNumber, 0 + toRemove))
      }
    }
  })
}
